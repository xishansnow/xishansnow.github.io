...
![](https://gitee.com/XiShanSnow/imagebed/raw/master/images/articles/word_embedding_097e6webp)

每个  $ C_i $  可能对应着不同的源语句子单词的注意力分配概率分布，比如对于上面的英汉翻译来说，其对应的信息可能如下：
$$
\begin{equation}|
\begin{aligned}
C_{\text {汤姆}}&=g(0.6 * f 2(\text{tom}), 0.2 * f 2( \text{chase}), 0.2 * f 2(  \text{jerry} )) \\\\
 C_{\text {追逐}}&=g(0.2 * f 2( \text{tom} ), 0.7 * f 2( \text{ chase} ), 0.1 * f 2( \text{ jerry} )) \\\\
 C( \text{ 杰瑞 } )&=g(0.3 * f 2( \text{ tom } ), 0.2 * f 2(\text{chase } ), 0.5 * f 2( \text{ jerry } )) 
 \end{aligned}
 \end{equation}
$$
...
![img](https://gitee.com/XiShanSnow/imagebed/raw/master/images/articles/word_embedding_5edffwebp)
...
![img](https://gitee.com/XiShanSnow/imagebed/raw/master/images/articles/word_embedding_ecc0fwebp)

在第一个阶段，可以引入不同的函数和计算机制，根据  $ Query $  和某个  $ Key_i $  ，计算两者的相似性或者相关性，最常见的方法包括：求两者的向量点积、求两者的向量余弦（Cosine）相似性或者通过再引入额外的神经网络来求值，即如下方式：

![](https://gitee.com/XiShanSnow/imagebed/raw/master/images/articles/word_embedding_6127dwebp)
...
![](https://gitee.com/XiShanSnow/imagebed/raw/master/images/articles/word_embedding_e6aacwebp)

第二阶段的计算结果  $ a_i $  即为  $ value_i $  对应的权重系数，然后进行加权求和即可得到  $ Attention $  数值：

![](https://gitee.com/XiShanSnow/imagebed/raw/master/images/articles/word_embedding_71476webp)
...
![img](https://gitee.com/XiShanSnow/imagebed/raw/master/images/articles/word_embedding_11792webp)

![img](https://gitee.com/XiShanSnow/imagebed/raw/master/images/articles/word_embedding_a99a2webp)
...
![](https://gitee.com/XiShanSnow/imagebed/raw/master/images/articles/word_embedding_ef98cwebp)
<center>图片-描述任务的Encoder-Decoder框架</center>
...
![](https://gitee.com/XiShanSnow/imagebed/raw/master/images/articles/word_embedding_00399webp)
<center>图片生成句子中每个单词时的注意力聚焦区域</center>
...
![](https://gitee.com/XiShanSnow/imagebed/raw/master/images/articles/word_embedding_da772webp)
<center> 图像描述任务中Attention机制的聚焦作用</center>
![](https://gitee.com/XiShanSnow/imagebed/raw/master/images/articles/word_embedding_79c52webp)
<center> 语音识别中音频序列和输出字符之间的Attention</center>
